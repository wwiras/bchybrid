...
def save_new_topology(gnewgraph, filename, k, end_time, clusters):
    ...
    fileout = f"{base_name}_kMeans{k}.json"
...
if __name__ == '__main__':
    ...
    parser.add_argument('--filename', type=str, required=True, help="Topology JSON file name")
    args = parser.parse_args()

    # Get total number of clusters
    k=int((args.cluster))

    # Get filename of BA/ER random network
    ...
    full_file_path = os.path.join(topology_folder,filename)
    
    # Check if the file exists
    if not os.path.exists(full_file_path):
        print(f"Error: File not found at '{full_file_path}'. Exiting.", flush=True)
        exit(1)

    # Load data from the JSON file (BA/ER random network)
    with open(full_file_path, 'r') as f: 
        data = json.load(f)

    # Build graph based on json (existing topology)
    G = nx.Graph()
    ...
    # Get cluster members in each cluster 
