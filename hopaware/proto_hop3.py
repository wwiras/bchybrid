...
NUM_NODES = 10         
M_INIT = 3             
FANOUT = 3             # MAXIMUM fanout allowed (k)
SIMULATION_ROUNDS = 50 
MAX_GOSSIP_HOPS = 5    

# --- REFINEMENT 1: ADJUSTED MEMORY (Decay Window) ---
# Medium memory to prevent quick cycles while allowing adaptation.
DECAY_WINDOW = 10      
...
class Node:
    ...
    def select_fanout_neighbors(self, current_hop, current_message_id):
        """
        Implements Hop-Aware Pruning (HAP) with Dynamic Fanout (DAF).
        The actual fanout is limited by the size of the 'good' neighbor pool.
        """
        ...
        # 2. Determine Good Pool
        nb_lists_candidate_good = nb_lists_all - nb_lists_bad_for_hop
        
        # --- CRUCIAL CHANGE: DYNAMIC FANOUT (DAF) ---
        # The actual fanout is the minimum of the maximum allowed FANOUT (3)
        # and the number of available good neighbors. 
        actual_fanout = min(FANOUT, len(nb_lists_candidate_good))
        
        if actual_fanout == 0:
            return set()
            
        # 3. RNS Selection from the Good List
        # We select 'actual_fanout' neighbors randomly from the good pool.
        nb_lists_target = set(random.sample(
            list(nb_lists_candidate_good), 
            k=actual_fanout
        ))
        
        # NOTE: The selection from the 'bad' list (Forced Exploration) has been removed.
        # This simplifies the protocol and saves bandwidth when pruning is effective.
...
print(f"--- Running DAF-Gossip (Dynamic Fanout) for {SIMULATION_ROUNDS} Rounds ---")
print(f"Starting Node: {start_node} (Hub), Max Fanout: {FANOUT}, Decay Window: {DECAY_WINDOW}")
print("-----------------------------------------------------------------------------")
...
fig.suptitle(f"DAF-Gossip Adaptation: Duplication vs. Coverage (Dynamic Fanout)\n"
             f"Network: BA({NUM_NODES}, M={M_INIT}), Max Fanout: {FANOUT}, Decay Window: {DECAY_WINDOW} Rounds", fontsize=16)
