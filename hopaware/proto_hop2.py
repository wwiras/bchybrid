...
# --- REFINEMENT 1: INCREASED MEMORY (Decay Window) ---
# A longer memory helps stabilize the duplication rate.
DECAY_WINDOW = 20      
...
class Node:
    ...
    def select_fanout_neighbors(self, current_hop, current_message_id):
        """
        Implements Hop-Aware Pruning (HAP) with Forced Exploration.
        """
        ...
        num_good = len(nb_lists_candidate_good)
        num_to_select_from_good = min(required_fanout, num_good)
        
        # Select from the good list first
        selected_good = set(random.sample(
            list(nb_lists_candidate_good), 
            k=num_to_select_from_good
        ))
        
        # --- REFINEMENT 2: FORCED EXPLORATION (Connectivity Fail-Safe) ---
        num_remaining_slots = required_fanout - num_to_select_from_good

        if num_remaining_slots > 0 and len(nb_lists_candidate_bad) > 0:
            # We must select from the bad list to meet the FANOUT requirement.
            
            # Select only from the bad neighbors who haven't already been picked 
            # (which shouldn't happen, but good safeguard)
            available_bad = list(nb_lists_candidate_bad) 
            
            # Select up to the remaining slots from the available bad list
        ...
        # Combine the final target list
        nb_lists_target = selected_good.union(selected_bad)
...
print(f"--- Running HAP-Gossip with Forced Exploration for {SIMULATION_ROUNDS} Rounds ---")
print(f"Starting Node: {start_node} (Hub), Fanout: {FANOUT}, Decay Window: {DECAY_WINDOW}")
print("-----------------------------------------------------------------------------------")
...
fig.suptitle(f"HAP-Gossip Adaptation: Duplication vs. Coverage (Forced Exploration)\n"
